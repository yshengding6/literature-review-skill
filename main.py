"""
文献调研综述 Skill (Socratic Roundtable Edition v4.0.0)
...
- 老丁因子（非共识观察点、冷知识命题）
- 多格式输出（md/pdf/docx）
- 乞醯章示例分析（examples/qixi-analysis/）
...
【精选案例】乞醯章分析示例见 examples/qixi-analysis/ 目录：
- 历代注疏原文整理（孔安国至程树德）
- 注疏六维解剖（朱熹、刘宝楠等）
- 诸子百家对比（儒道墨法）
- 东西方哲学对比（苏格拉底/康德/亚里士多德）
- 苏格拉底圆桌会议纪要
- 完整文献综述报告（含老丁因子）
"""
...
class CrossVerifier:
    ...
    def analyze_consensus(
        ...
        for point1 in all_points[:20]:
            all_opp_words = [word for pair in opposition_pairs for word in pair]
            if any(opp in point1['content'].lower() for opp in all_opp_words):
                # 查找对立观点
...
if MCP_AVAILABLE and mcp:
    @mcp.tool()
    def get_qixi_example() -> Dict[str, Any]:
        """
        获取乞醯章分析示例信息

        Returns:
            乞醯章示例的完整信息，包括标题、原文、核心发现、文件列表等
        """
        return {
            "title": QIXI_EXAMPLE["title"],
            "chapter": QIXI_EXAMPLE["chapter"],
            "description": QIXI_EXAMPLE["description"],
            "key_findings": QIXI_EXAMPLE["key_findings"],
            "cold_knowledge": QIXI_EXAMPLE["cold_knowledge"],
            "laoding_factor": QIXI_EXAMPLE["laoding_factor"],
            "files": QIXI_EXAMPLE["files"],
            "note": "这是一个展示四把手术刀方法论（政治史探针、六维解剖、异常检测、变量替换）的完整案例"
        }
...
# 乞醯章分析示例数据
QIXI_EXAMPLE = {
    "title": "《论语·公冶长第五·乞醯章》深度分析",
    "chapter": "孰谓微生高直？或乞醯焉，乞诸其邻而与之。",
    "description": "这是一个展示四把手术刀方法论的完整案例",
    "files": [
        "examples/qixi-analysis/01-历代注疏原文.md",
        "examples/qixi-analysis/02-注疏六维解剖.md",
        "examples/qixi-analysis/03-诸子百家对比.md",
        "examples/qixi-analysis/04-东西方哲学对比.md",
        "examples/qixi-analysis/05-圆桌会议纪要.md",
        "examples/qixi-analysis/乞醯章_文献综述报告.md"
    ],
    "key_findings": [
        "孔子不是在批评微生高，而是在质疑'直'这个标签本身",
        "历代注疏呈现从'道德评判'到'政治博弈'再到'语言哲学'的演进",
        "'乞醯'二字的选择具有不可替代的指向性",
        "微生高可能是春秋时期的'社交润滑剂'式人物"
    ],
    "cold_knowledge": "微生高不是不直，而是'直'这个概念本身就不足以描述人性的复杂。",
    "laoding_factor": "我们今天的社交媒体，依然在重复'乞醯章'揭示的困境：用简单的道德标签（直/不直、好/坏）来评判复杂的人性。"
}
...
def main():
    ...
    if MCP_AVAILABLE and mcp and len(sys.argv) == 1:
        ...
        # 独立运行模式
        print("=== 文献调研综述工具 (Socratic Roundtable Edition v4.0.0) ===")
        print()
        print("使用方式:")
        print("  1. 作为 MCP Server 运行: python main.py")
        print("  2. 作为独立脚本运行: python main.py --topic <主题> [--files 文件路径...]")
        print("  3. 查看乞醯章示例: python main.py --example-qixi")
        print()

        # 乞醯章示例展示
        if "--example-qixi" in sys.argv or "--qixi" in sys.argv:
            print("=" * 60)
            print(f"📜 {QIXI_EXAMPLE['title']}")
            print("=" * 60)
            print()
            print(f"原文：{QIXI_EXAMPLE['chapter']}")
            print()
            print(f"说明：{QIXI_EXAMPLE['description']}")
            print()
            print("核心发现：")
            for i, finding in enumerate(QIXI_EXAMPLE['key_findings'], 1):
                print(f"  {i}. {finding}")
            print()
            print(f"冷知识命题：{QIXI_EXAMPLE['cold_knowledge']}")
            print()
            print(f"老丁因子：{QIXI_EXAMPLE['laoding_factor']}")
            print()
            print("相关文件：")
            for f in QIXI_EXAMPLE['files']:
                print(f"  - {f}")
            print()
            print("=" * 60)
            print("提示：使用 --topic '乞醯章' --files examples/qixi-analysis/*.md")
            print("      可以加载这些文件进行分析")
            print("=" * 60)
            return
        ...
        if "--topic" in sys.argv:
            ...
            print("请提供 --topic 参数或使用 --example-qixi 查看示例")
            print()
            print("常用命令：")
            print("  python main.py --example-qixi              # 查看乞醯章示例")
            print("  python main.py --topic '人工智能医疗'       # 基础分析")
            print("  python main.py --topic '乞醯章' --files examples/qixi-analysis/*.md")
            print("  python main.py --topic '深度学习' --files paper1.pdf paper2.pdf --lang en")
            print()
            print("参数说明：")
            print("  --topic <主题>        研究主题（必填）")
            print("  --files <文件...>     本地文档路径")
            print("  --lang <zh|en>        输出语言（默认：zh）")
            print("  --depth <basic|medium|deep>  搜索深度（默认：medium）")
            print("  --no-web              禁用网络搜索")
            print("  --no-bibtex           禁用BibTeX导出")
            print("  --simple-read         简单读取模式（仅显示文件内容）")
